...
def _load_schedule(year: int) -> dict:
...
def scrape_season(year: int) -> list[Game]:
...
def scrape_all_seasons(start_year: int, end_year: int) -> list[Game]:
    """Scrape all games across a range of seasons (inclusive end years)."""
    games = []
    for year in range(start_year, end_year + 1):
        games.extend(scrape_season(year))
    return games
